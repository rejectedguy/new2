    ...
    try {
        ...
        for (i = start_index; i < end_index; i++) {
            ...
            try {
                ...
                // Добавляем regional_name и regional_parent_id для группировки
                // Проверяем, является ли подразделение частью группы (имеет скрытого родителя)
                // Добавляем regional_name и regional_parent_id для группировки
// РЕКУРСИВНО поднимаемся вверх по иерархии, пока не найдем скрытое подразделение
...
try {
    ...
    if (ArrayCount(col_sub_detail_result) > 0) {
        // РЕКУРСИВНЫЙ ПОИСК скрытого родителя
        ...
        while (check_depth < 10 && !found_regional) {
            ...
            if (ArrayCount(col_parent_result) > 0) {
                ...
                // Если нашли скрытое подразделение - это региональное
...
for (i = 0; i < ArrayCount(grouped_ids); i++) {
    ...
    try {
        ...
        regional_parent_id = String(dept_info.parent_object_id);
        regional_name = '';
regional_parent_id_str = '';
try {
    current_check_sub = dept_info;
    search_depth = 0;
    found_territorial = false;
    
    while (search_depth < 10 && !found_territorial) {
        search_depth++;
        ...
        if (ArrayCount(check_parent_result) > 0) {
            ...
            if (startsWithHiddenPrefix(check_parent_name)) {
                ...
                result.GetOptProperty('debug').push('Found territorial for parent dept ' + dept_id + ' at depth ' + search_depth + ': ' + regional_name);
                break;
            }
    ...
    if (!found_territorial) {
        ...
        result.GetOptProperty('debug').push('No territorial found for parent dept ' + dept_id);
    }
} catch(regErr) {
    regional_name = 'Неизвестное подразделение';
    result.GetOptProperty('debug').push('Error finding territorial for parent: ' + String(regErr));
}
    ...
    // НОВАЯ ЛОГИКА: Проверяем сотрудников В ПОДРАЗДЕЛЕНИИ ИЛИ В ЕГО ДЕТЯХ
...
for (norm_key in global_normalized_map) {
    try {
        ...
        if (ArrayCount(group_ids) > 1) {
            ...
            child_data.SetProperty('org_id', group_info.org_id); // Добавляем org_id
        ...
        //all_children_array = ArraySort(all_children_array, 'name', '+');
...
try {
    ...
    result.GetOptProperty('debug').push('Error finding territorial for parent: ' + String(regErr));
        ...
        // ===== НОВЫЙ КОД: Собираем дочерние подразделения (уровень 1) =====
        ...
        // ===== Собираем func_manager'ов для ДОЧЕРНИХ подразделений (уровень 1) =====
        for (dept_child in dept_children) {
            try {
                child_dept_id = String(dept_child.id);
                child_fm_id = getFuncManagerForSubdivision(child_dept_id, true);
                
                if (child_fm_id != '') {
                    child_fm_query = "for $elem in collaborators where $elem/id = " + child_fm_id + " and $elem/is_dismiss = false() return $elem";
                    child_fm_result = ArraySelectAll(tools.xquery(child_fm_query));
                    
                    if (ArrayCount(child_fm_result) > 0) {
                        child_fm_elem = child_fm_result[0];
                        
                        // Получаем имя дочернего подразделения
                        child_dept_name = getSubNameById(child_dept_id);
                        if (child_dept_name == null) {
                            child_dept_name = String(dept_child.name);
                        }
                        
                        // Очищаем от точек
                        clean_child_dept_name = child_dept_name;
                        while (StrContains(clean_child_dept_name, '.', false)) {
                            clean_child_dept_name = StrReplace(clean_child_dept_name, '.', '');
                        }
                        clean_child_dept_name = Trim(clean_child_dept_name);
                        child_dept_normalized = normalizeSubdivisionName(clean_child_dept_name);
                        
                        if (!collaborators_by_normalized_name.HasProperty(child_dept_normalized)) {
                            collaborators_by_normalized_name.SetProperty(child_dept_normalized, []);
                            normalized_names_order.push(child_dept_normalized);
                        }
                        
                        // Проверяем дубль в конкретном подразделении
                        child_dept_collaborators = collaborators_by_normalized_name.GetOptProperty(child_dept_normalized);
                        fm_already_in_this_dept = false;
                        for (check_idx = 0; check_idx < ArrayCount(child_dept_collaborators); check_idx++) {
                            if (String(child_dept_collaborators[check_idx].GetOptProperty('id', '')) == child_fm_id) {
                                fm_already_in_this_dept = true;
                                break;
                            }
                        }
                        
                        if (!fm_already_in_this_dept) {
                            fm_elem_copy = new Object();
                            fm_elem_copy.SetProperty('id', String(child_fm_elem.id));
                            fm_elem_copy.SetProperty('fullname', String(child_fm_elem.fullname));
                            fm_elem_copy.SetProperty('position_parent_id', String(child_fm_elem.position_parent_id));
                            fm_elem_copy.SetProperty('email', child_fm_elem.email);
                            fm_elem_copy.SetProperty('pict_url', child_fm_elem.pict_url);
                            fm_elem_copy.SetProperty('position_name', child_fm_elem.position_name);
                            fm_elem_copy.SetProperty('mobile_phone', child_fm_elem.mobile_phone);
                            fm_elem_copy.SetProperty('phone', child_fm_elem.phone);
                            fm_elem_copy.SetProperty('birth_date', child_fm_elem.birth_date);
                            fm_elem_copy.SetProperty('current_state', child_fm_elem.current_state);
                            fm_elem_copy.SetProperty('is_func_manager_flag', true);
                            fm_elem_copy.SetProperty('regional_name', regional_name);
                            fm_elem_copy.SetProperty('dept_name', clean_child_dept_name);
                            fm_elem_copy.SetProperty('dept_name_normalized', child_dept_normalized);
                            
                            collaborators_by_normalized_name.GetOptProperty(child_dept_normalized).push(fm_elem_copy);
                            
                            result.GetOptProperty('debug').push('Added child dept FM (level 1): ' + child_fm_id + ' for dept: ' + clean_child_dept_name);
                        }
                    }
                }
            } catch(childFmErr) {
                result.GetOptProperty('debug').push('ERROR loading child dept FM: ' + String(childFmErr));
            }
        }
        
        // ===== Собираем func_manager'ов для ВНУКОВ (уровень 2) =====
        for (dept_gc in dept_grandchildren) {
            try {
                gc_dept_id = String(dept_gc.id);
                gc_fm_id = getFuncManagerForSubdivision(gc_dept_id, true);
                
                if (gc_fm_id != '') {
                    gc_fm_query = "for $elem in collaborators where $elem/id = " + gc_fm_id + " and $elem/is_dismiss = false() return $elem";
                    gc_fm_result = ArraySelectAll(tools.xquery(gc_fm_query));
                    
                    if (ArrayCount(gc_fm_result) > 0) {
                        gc_fm_elem = gc_fm_result[0];
                        
                        // Получаем имя внука
                        gc_dept_name = getSubNameById(gc_dept_id);
                        if (gc_dept_name == null) {
                            gc_dept_name = String(dept_gc.name);
                        }
                        
                        // Очищаем от точек
                        clean_gc_dept_name = gc_dept_name;
                        while (StrContains(clean_gc_dept_name, '.', false)) {
                            clean_gc_dept_name = StrReplace(clean_gc_dept_name, '.', '');
                        }
                        clean_gc_dept_name = Trim(clean_gc_dept_name);
                        gc_dept_normalized = normalizeSubdivisionName(clean_gc_dept_name);
                        
                        if (!collaborators_by_normalized_name.HasProperty(gc_dept_normalized)) {
                            collaborators_by_normalized_name.SetProperty(gc_dept_normalized, []);
                            normalized_names_order.push(gc_dept_normalized);
                        }
                        
                        // Проверяем дубль в конкретном подразделении
                        gc_dept_collaborators = collaborators_by_normalized_name.GetOptProperty(gc_dept_normalized);
                        fm_already_in_this_dept = false;
                        for (check_idx = 0; check_idx < ArrayCount(gc_dept_collaborators); check_idx++) {
                            if (String(gc_dept_collaborators[check_idx].GetOptProperty('id', '')) == gc_fm_id) {
                                fm_already_in_this_dept = true;
                                break;
                            }
                        }
                        ...
                        if (!fm_already_in_this_dept) {
                            gc_fm_elem_copy = new Object();
                            gc_fm_elem_copy.SetProperty('id', String(gc_fm_elem.id));
                            gc_fm_elem_copy.SetProperty('fullname', String(gc_fm_elem.fullname));
                            gc_fm_elem_copy.SetProperty('position_parent_id', String(gc_fm_elem.position_parent_id));
                            gc_fm_elem_copy.SetProperty('email', gc_fm_elem.email);
                            gc_fm_elem_copy.SetProperty('pict_url', gc_fm_elem.pict_url);
                            gc_fm_elem_copy.SetProperty('position_name', gc_fm_elem.position_name);
                            gc_fm_elem_copy.SetProperty('mobile_phone', gc_fm_elem.mobile_phone);
                            gc_fm_elem_copy.SetProperty('phone', gc_fm_elem.phone);
                            gc_fm_elem_copy.SetProperty('birth_date', gc_fm_elem.birth_date);
                            gc_fm_elem_copy.SetProperty('current_state', gc_fm_elem.current_state);
                            gc_fm_elem_copy.SetProperty('is_func_manager_flag', true);
                            gc_fm_elem_copy.SetProperty('regional_name', regional_name);
                            gc_fm_elem_copy.SetProperty('dept_name', clean_gc_dept_name);
                            gc_fm_elem_copy.SetProperty('dept_name_normalized', gc_dept_normalized);
                            
                            collaborators_by_normalized_name.GetOptProperty(gc_dept_normalized).push(gc_fm_elem_copy);
                            
                            result.GetOptProperty('debug').push('Added grandchild dept FM (level 2): ' + gc_fm_id + ' for dept: ' + clean_gc_dept_name);
                        }
                    }
                }
            } catch(gcFmErr) {
                result.GetOptProperty('debug').push('ERROR loading grandchild dept FM: ' + String(gcFmErr));
            }
        }
        
       func_manager_id = getFuncManagerForSubdivision(dept_id, true);
        
        if (func_manager_id != '' && !added_person_ids.HasProperty(func_manager_id)) {
            try {
                func_manager_full_query = "for $elem in collaborators where $elem/id = " + func_manager_id + " and $elem/is_dismiss = false() return $elem";
                func_manager_data_result = ArraySelectAll(tools.xquery(func_manager_full_query));
                ...
                if (ArrayCount(func_manager_data_result) > 0) {
                    fm_elem = func_manager_data_result[0];
                    fm_elem_copy = new Object();
                    fm_elem_copy.SetProperty('id', String(fm_elem.id));
                    fm_elem_copy.SetProperty('fullname', String(fm_elem.fullname));
                    fm_elem_copy.SetProperty('position_parent_id', String(fm_elem.position_parent_id));
                    fm_elem_copy.SetProperty('email', fm_elem.email);
                    fm_elem_copy.SetProperty('pict_url', fm_elem.pict_url);
                    fm_elem_copy.SetProperty('position_name', fm_elem.position_name);
                    fm_elem_copy.SetProperty('mobile_phone', fm_elem.mobile_phone);
                    fm_elem_copy.SetProperty('phone', fm_elem.phone);
                    fm_elem_copy.SetProperty('birth_date', fm_elem.birth_date);
                    fm_elem_copy.SetProperty('current_state', fm_elem.current_state);
                    fm_elem_copy.SetProperty('is_func_manager_flag', true);
                    fm_elem_copy.SetProperty('regional_name', regional_name);
                    fm_elem_copy.SetProperty('dept_name', clean_dept_name);
                    fm_elem_copy.SetProperty('dept_name_normalized', dept_name_normalized);
                    
                    collaborators_by_normalized_name.GetOptProperty(dept_name_normalized).push(fm_elem_copy);
                    added_person_ids.SetProperty(func_manager_id, true);
                }
            } catch(fmErr) {}
        }
        
// Обычные сотрудники родительского dept
dept_col_query = "for $elem in collaborators where $elem/position_parent_id = " + dept_id + " and $elem/is_dismiss = false() return $elem";
dept_cols = ArraySelectAll(tools.xquery(dept_col_query));

for (col in dept_cols) {
    try {
        col_id_str = String(col.id);
        
        if (added_person_ids.HasProperty(col_id_str)) {
            continue;
        }
        
        // НОВАЯ ПРОВЕРКА: Проверяем, не является ли этот человек уже FM в этом подразделении
        dept_collaborators = collaborators_by_normalized_name.GetOptProperty(dept_name_normalized);
        is_already_fm_in_dept = false;
        for (check_idx = 0; check_idx < ArrayCount(dept_collaborators); check_idx++) {
            existing_col = dept_collaborators[check_idx];
            if (String(existing_col.GetOptProperty('id', '')) == col_id_str && 
                existing_col.GetOptProperty('is_func_manager_flag', false) == true) {
                is_already_fm_in_dept = true;
                result.GetOptProperty('debug').push('Skipping duplicate: ' + col_id_str + ' already FM in ' + dept_name_normalized);
                break;
            }
        }
        
        if (is_already_fm_in_dept) {
            continue;
        }
        
        col_copy = new Object();
        col_copy.SetProperty('id', col_id_str);
        col_copy.SetProperty('fullname', String(col.fullname));
        col_copy.SetProperty('position_parent_id', String(col.position_parent_id));
        col_copy.SetProperty('email', col.email);
        col_copy.SetProperty('pict_url', col.pict_url);
        col_copy.SetProperty('position_name', col.position_name);
        col_copy.SetProperty('mobile_phone', col.mobile_phone);
        col_copy.SetProperty('phone', col.phone);
        col_copy.SetProperty('birth_date', col.birth_date);
        col_copy.SetProperty('current_state', col.current_state);
        col_copy.SetProperty('is_func_manager_flag', false);
        col_copy.SetProperty('regional_name', regional_name);
        col_copy.SetProperty('regional_parent_id', regional_parent_id_str);
        col_copy.SetProperty('dept_name', clean_dept_name);
        col_copy.SetProperty('dept_name_normalized', dept_name_normalized);
        
        // ВАЖНО: Сохраняем оригинальное dept_id для группировки внутри региона
        col_copy.SetProperty('original_dept_id', dept_id);
        
        collaborators_by_normalized_name.GetOptProperty(dept_name_normalized).push(col_copy);
        added_person_ids.SetProperty(col_id_str, true);
    } catch(colErr) {}
}
        
// ===== НОВЫЙ КОД: Собираем сотрудников из дочерних подразделений (уровень 1) =====
for (dept_child in dept_children) {
    try {
        child_dept_id = String(dept_child.id);
        ...
        child_col_query = "for $elem in collaborators where $elem/position_parent_id = " + child_dept_id + " and $elem/is_dismiss = false() return $elem";
        child_cols = ArraySelectAll(tools.xquery(child_col_query));
        ...
        for (col in child_cols) {
            try {
                ...
                is_already_fm_in_dept = false;
                for (check_idx = 0; check_idx < ArrayCount(child_dept_collaborators); check_idx++) {
                    existing_col = child_dept_collaborators[check_idx];
                    if (String(existing_col.GetOptProperty('id', '')) == col_id_str && 
                        existing_col.GetOptProperty('is_func_manager_flag', false) == true) {
                        is_already_fm_in_dept = true;
                        result.GetOptProperty('debug').push('Skipping duplicate: ' + col_id_str + ' already FM in ' + child_dept_normalized);
                        break;
                    }
                }
                ...
                col_copy.SetProperty('regional_name', regional_name);
                col_copy.SetProperty('regional_parent_id', regional_parent_id_str);
                col_copy.SetProperty('dept_name', clean_child_dept_name);
                col_copy.SetProperty('dept_name_normalized', child_dept_normalized);
                
                // ВАЖНО: Сохраняем original_dept_id для группировки
                col_copy.SetProperty('original_dept_id', child_dept_id);
                
                collaborators_by_normalized_name.GetOptProperty(child_dept_normalized).push(col_copy);
                added_person_ids.SetProperty(col_id_str, true);
            } catch(colErr) {}
        }
    } catch(childColErr) {
        result.GetOptProperty('debug').push('Error loading child dept collaborators: ' + String(childColErr));
    }
}
...
for (dept_gc in dept_grandchildren) {
    try {
        ...
        gc_col_query = "for $elem in collaborators where $elem/position_parent_id = " + gc_dept_id + " and $elem/is_dismiss = false() return $elem";
        gc_cols = ArraySelectAll(tools.xquery(gc_col_query));
        ...
        for (col in gc_cols) {
            try {
                ...
                is_already_fm_in_dept = false;
                for (check_idx = 0; check_idx < ArrayCount(gc_dept_collaborators); check_idx++) {
                    existing_col = gc_dept_collaborators[check_idx];
                    if (String(existing_col.GetOptProperty('id', '')) == col_id_str && 
                        existing_col.GetOptProperty('is_func_manager_flag', false) == true) {
                        is_already_fm_in_dept = true;
                        result.GetOptProperty('debug').push('Skipping duplicate: ' + col_id_str + ' already FM in ' + gc_dept_normalized);
                        break;
                    }
                }
               ...
               col_copy.SetProperty('is_func_manager_flag', false);
                col_copy.SetProperty('regional_name', regional_name);
                col_copy.SetProperty('regional_parent_id', regional_parent_id_str);
                col_copy.SetProperty('dept_name', clean_gc_dept_name);
                col_copy.SetProperty('dept_name_normalized', gc_dept_normalized);
                
                // ВАЖНО: Сохраняем original_dept_id для группировки
                col_copy.SetProperty('original_dept_id', gc_dept_id);
                
                collaborators_by_normalized_name.GetOptProperty(gc_dept_normalized).push(col_copy);
                added_person_ids.SetProperty(col_id_str, true);
            } catch(colErr) {}
        }
    } catch(gcColErr) {
        result.GetOptProperty('debug').push('Error loading grandchild dept collaborators: ' + String(gcColErr));
    }
}
        
        result.GetOptProperty('debug').push('Parent dept "' + clean_dept_name + '" loaded with children and grandchildren');
...
result.GetOptProperty('debug').push('=== NOW LOADING CHILD SUBDIVISIONS ===');
for (norm_key in global_normalized_map) {
    try {
        group_info = global_normalized_map.GetOptProperty(norm_key);
        group_ids = group_info.ids;
        clean_group_name = group_info.display_name;
        
        result.GetOptProperty('debug').push('Loading collaborators for: "' + clean_group_name + '" (' + ArrayCount(group_ids) + ' subs)');
        
        if (!collaborators_by_normalized_name.HasProperty(norm_key)) {
            collaborators_by_normalized_name.SetProperty(norm_key, []);
            normalized_names_order.push(norm_key);
        }
...
try {
    ...
    result.GetOptProperty('debug').push('Error finding territorial for parent: ' + String(regErr));
            ...
            func_manager_id = getFuncManagerForSubdivision(sub_id, true);
            
            if (func_manager_id != '') {  // УБРАЛИ проверку !added_person_ids.HasProperty
    try {
        func_manager_full_query = "for $elem in collaborators where $elem/id = " + func_manager_id + " and $elem/is_dismiss = false() return $elem";
        func_manager_data_result = ArraySelectAll(tools.xquery(func_manager_full_query));
        
        if (ArrayCount(func_manager_data_result) > 0) {
            // ПРОВЕРЯЕМ: уже добавлен ли FM в ЭТОТ norm_key
            current_dept_collaborators = collaborators_by_normalized_name.GetOptProperty(norm_key);
            fm_already_in_this_dept = false;
            for (check_idx = 0; check_idx < ArrayCount(current_dept_collaborators); check_idx++) {
                if (String(current_dept_collaborators[check_idx].GetOptProperty('id', '')) == func_manager_id) {
                    fm_already_in_this_dept = true;
                    break;
                }
            }
            ...
            if (!fm_already_in_this_dept) {
                ...
                fm_elem_copy.SetProperty('is_func_manager_flag', true);
                fm_elem_copy.SetProperty('regional_name', regional_name);
                fm_elem_copy.SetProperty('dept_name', clean_group_name);
                fm_elem_copy.SetProperty('dept_name_normalized', norm_key);
                
                collaborators_by_normalized_name.GetOptProperty(norm_key).push(fm_elem_copy);
                // НЕ ДОБАВЛЯЕМ в added_person_ids - позволяем дубли для FM в разных подразделениях
            }
        }
    } catch(fmErr) {}
}
            
            sub_col_query = "for $elem in collaborators where $elem/position_parent_id = " + sub_id + " and $elem/is_dismiss = false() return $elem";
            sub_cols = ArraySelectAll(tools.xquery(sub_col_query));
        ...
        is_already_fm_in_dept = false;
        for (check_idx = 0; check_idx < ArrayCount(current_dept_collaborators); check_idx++) {
            existing_col = current_dept_collaborators[check_idx];
            if (String(existing_col.GetOptProperty('id', '')) == col_id_str && 
                existing_col.GetOptProperty('is_func_manager_flag', false) == true) {
                is_already_fm_in_dept = true;
                result.GetOptProperty('debug').push('Skipping duplicate: ' + col_id_str + ' already FM in ' + norm_key);
                break;
            }
        }
        ...
        if (is_already_fm_in_dept) {
            continue;
                   ...
                   col_copy.SetProperty('is_func_manager_flag', false);
                    col_copy.SetProperty('regional_name', regional_name);
                    col_copy.SetProperty('regional_parent_id', regional_parent_id_str);
                    col_copy.SetProperty('dept_name', clean_group_name);
                    col_copy.SetProperty('dept_name_normalized', norm_key);
                    
                    // ВАЖНО: Сохраняем sub_id как original_dept_id
                    col_copy.SetProperty('original_dept_id', sub_id);
                    
                    collaborators_by_normalized_name.GetOptProperty(norm_key).push(col_copy);
                    added_person_ids.SetProperty(col_id_str, true);
                } catch(colErr) {}
            }
        }
        ...
        temp_all_grouped = [];
        for (i = 0; i < ArrayCount(normalized_names_order); i++) {
            normalized_key = normalized_names_order[i];
            sub_cols = collaborators_by_normalized_name.GetOptProperty(normalized_key);
            
            for (col in sub_cols) {
                temp_all_grouped.push(col);
            }
        }
        
        total_grouped = ArrayCount(temp_all_grouped);
		// Пагинация
        start_index = offset_param;
        end_index = offset_param + limit_param;
        if (end_index > total_grouped) end_index = total_grouped;
        
        paginated_grouped = [];
        for (i = start_index; i < end_index; i++) {
            col_elem = temp_all_grouped[i];
            
            try {
                if (!col_elem.id || col_elem.id == '') continue;
                
                is_birthday = checkBirthday(col_elem.GetOptProperty('birth_date', null));
                is_on_vacation = (String(col_elem.GetOptProperty('current_state', '')) == 'Отпуск');
                is_func_mgr = col_elem.GetOptProperty('is_func_manager_flag', false);
                
                col_subdivision_name = col_elem.GetOptProperty('dept_name', '');
                
                // УБИРАЕМ ТОЧКИ из названия для отображения
                while (StrContains(col_subdivision_name, '.', false)) {
                    col_subdivision_name = StrReplace(col_subdivision_name, '.', '');
                }
                col_subdivision_name = Trim(col_subdivision_name);
                sub_id_for_name = String(col_elem.GetOptProperty('position_parent_id', ''));
                
                if (col_subdivision_name == '') {
                    col_sub_query = "for $elem in subdivisions where $elem/id = " + sub_id_for_name + " return $elem";
                    col_sub_result = ArraySelectAll(tools.xquery(col_sub_query));
                    if (ArrayCount(col_sub_result) > 0) {
                        colSubName = getSubNameById(sub_id_for_name);
                        if (colSubName == null) {
                            colSubName = String(col_sub_result[0].name);
                        }
                        col_subdivision_name = colSubName;
                ...
                col_data = new Object();
                col_data.SetProperty('id', String(col_elem.GetOptProperty('id', '')));
                col_data.SetProperty('name', String(col_elem.GetOptProperty('fullname', '')));
                col_data.SetProperty('name_lower', StrLowerCase(String(col_elem.GetOptProperty('fullname', ''))));
                col_data.SetProperty('subdivision_id', sub_id_for_name);
                col_data.SetProperty('subdivision_name', col_subdivision_name);
                col_data.SetProperty('email', String(col_elem.GetOptProperty('email', null) != null ? col_elem.GetOptProperty('email', '') : '—'));
                col_data.SetProperty('pict_url', String(col_elem.GetOptProperty('pict_url', null) != null ? col_elem.GetOptProperty('pict_url', '') : ''));
                col_data.SetProperty('position_name', String(col_elem.GetOptProperty('position_name', null) != null ? col_elem.GetOptProperty('position_name', '') : '—'));
                col_data.SetProperty('mobile_phone', String(col_elem.GetOptProperty('mobile_phone', null) != null ? col_elem.GetOptProperty('mobile_phone', '') : '—'));
                col_data.SetProperty('phone', String(col_elem.GetOptProperty('phone', null) != null ? col_elem.GetOptProperty('phone', '') : '—'));
                col_data.SetProperty('is_birthday', is_birthday);
                col_data.SetProperty('is_on_vacation', is_on_vacation);
                col_data.SetProperty('is_func_manager', is_func_mgr);
                col_data.SetProperty('dept_name_normalized', String(col_elem.GetOptProperty('dept_name_normalized', col_subdivision_name)));
                col_data.SetProperty('regional_name', String(col_elem.GetOptProperty('regional_name', '')));
                col_data.SetProperty('regional_parent_id', String(col_elem.GetOptProperty('regional_parent_id', '')));
                col_data.SetProperty('original_dept_id', String(col_elem.GetOptProperty('original_dept_id', '')));
                paginated_grouped.push(col_data);
            } catch(colErr) {}
        }
        
        subdivision_data.SetProperty('all_collaborators', paginated_grouped);
        subdivision_data.SetProperty('total_count', total_grouped);
        subdivision_data.SetProperty('current_offset', offset_param);
        subdivision_data.SetProperty('current_limit', limit_param);
        subdivision_data.SetProperty('has_more', end_index < total_grouped);
    } catch(allCollabErr) {
...
Response.Write(tools.object_to_text(result, 'json'));
        ...
        // Проверяем является ли это подразделение частью группы (имеет скрытого родителя)
// Проверяем является ли это подразделение частью группы (имеет скрытого родителя ИЛИ ДЕДУШКУ)
...
try {
    ...
    if (ArrayCount(current_sub_result) > 0) {
        // Проверяем ЦЕПОЧКУ родителей до 3 уровней вверх
        ...
        while (check_parent_id != '' && check_parent_id != null && depth_check < 3) {
            ...
            if (ArrayCount(parent_sub_result) > 0) {
                ...
                // Проверяем является ли родитель скрытым
...
// Проверяем является ли родитель текущего подразделения grouped элементом
// Проверяем является ли родитель или дедушка текущего подразделения grouped элементом
    ...
    if (ArrayCount(current_sub_result) > 0) {
        ...
        if (parent_sub_id != '' && parent_sub_id != null) {
            ...
            if (ArrayCount(parent_sub_result) > 0) {
                ...
                // ПРОВЕРКА 1: Если непосредственный родитель скрытый
                if (startsWithHiddenPrefix(parent_sub_name)) {
                    ...
                    // ПРОВЕРКА 2: Проверяем дедушку (родителя родителя)
                    ...
                    if (grandparent_sub_id != '' && grandparent_sub_id != null) {
                        ...
                        if (ArrayCount(grandparent_sub_result) > 0) {
                            ...
                            // Если дедушка скрытый - это тоже часть grouped структуры
...
// Если подразделение часть группы - используем ТОЛЬКО subdivision catalog
...
// Добавляем функционального менеджера в temp_all, если он существует
if (func_manager_id != '') {
    try {
        func_manager_full_query = "for $elem in collaborators where $elem/id = " + func_manager_id + " and $elem/is_dismiss = false() return $elem";
        func_manager_data_result = ArraySelectAll(tools.xquery(func_manager_full_query));
        ...
        if (ArrayCount(func_manager_data_result) > 0) {
            fm_elem = func_manager_data_result[0];
            temp_all.push(fm_elem); // Добавляем функционального менеджера в temp_all
            result.GetOptProperty('debug').push('Added func manager to temp_all: ' + func_manager_id);
        }
    } catch(fmErr) {
        result.GetOptProperty('debug').push('Error adding func manager to temp_all: ' + String(fmErr));
    }
        ...
        children_array = [];
		// Загружаем дочерние подразделения для получения их руководителей
...
// Собираем руководителей дочерних подразделений
child_func_managers_map = new Object();
for (child_sub in child_subdivisions_list) {
    try {
        child_sub_id = String(child_sub.id);
        // Проверяем является ли дочернее подразделение частью группы
is_child_part_of_group = false;
if (is_part_of_group) {
    // Если родитель в группе, то и дети в группе
    is_child_part_of_group = true;
} else {
    // Проверяем родителя дочернего подразделения
    try {
...
result.GetOptProperty('debug').push('Final grouped_ids_str for ' + normalized_key + ' = ' + grouped_ids_str + ', length=' + StrLen(grouped_ids_str));
        ...
        //children_array = ArraySort(children_array, 'name', '+');
...
for (col_elem in col_list) {
    try {
        ...
        // Пропускаем если это руководитель родительского подразделения
        if (func_manager_id != '' && col_elem_id == func_manager_id) continue;
        
        // Проверяем является ли сотрудник руководителем дочернего подразделения
        is_child_fm = child_func_managers_map.HasProperty(col_elem_id);
                ...
                // НОВАЯ ЛОГИКА: Всегда проверяем региональную принадлежность
                try {
                    col_pos_parent_id = String(col_elem.position_parent_id);
                    ...
                    if (ArrayCount(col_sub_detail_result) > 0) {
                        col_sub_detail = col_sub_detail_result[0];
                        col_parent_id = String(col_sub_detail.parent_object_id);
                        
                        // Проверяем родителя
                        if (col_parent_id != '' && col_parent_id != null) {
                            col_parent_query = "for $elem in subdivisions where $elem/id = " + col_parent_id + " return $elem";
                            col_parent_result = ArraySelectAll(tools.xquery(col_parent_query));
                            ...
                            if (ArrayCount(col_parent_result) > 0) {
                                ...
                                // Если родитель скрытый - это региональное подразделение
                                if (startsWithHiddenPrefix(col_parent_name)) {
                                    col_data.SetProperty('regional_name', col_parent_name);
                                    col_data.SetProperty('regional_parent_id', col_parent_id);
                                    col_data.SetProperty('original_dept_id', col_pos_parent_id);
        ...
        // НОВАЯ ЛОГИКА: Если parent_is_grouped, группируем по региональным подразделениям
        if (parent_is_grouped) {
            result.GetOptProperty('debug').push('Applying regional grouping for child subdivision');
            
            // Группируем по regional_parent_id
            ...
            // Пересоздаем collaborators_array с региональными заголовками
            collaborators_array = [];
            ...
            for (i = 0; i < ArrayCount(regional_order); i++) {
                ...
                // Разделяем на менеджеров и обычных внутри региона
                regional_managers = [];
                regional_regular = [];
...
if (!has_collab_top) continue;
    ...
    try {
        ...
        // СНАЧАЛА проверяем есть ли реально дети
        has_real_children = false;
        for (j = 0; j < ArrayCount(dept_group); j++) {
            if (hasSubdivisionsWithCollaborators(String(dept_group[j].id))) {
                has_real_children = true;
        ...
        grouped_data.SetProperty('is_grouped', true);
        grouped_data.SetProperty('group_count', ArrayCount(dept_group));
        grouped_data.SetProperty('has_subdivisions', has_real_children);  // <--- ИЗМЕНЕНО
                    grouped_data.SetProperty('grouped_items_str', grouped_ids_str);
                    grouped_data.SetProperty('children', []);
                    grouped_data.SetProperty('collaborators', []);
...
try {
    // Строим путь для этого сотрудника
    full_path_items = [];
    ...
    while (current_sub_id != '' && current_sub_id != null && path_depth < 20) {
        ...
        if (sub_path_info.parent_object_id != '' && sub_path_info.parent_object_id != null) {
            ...
            // Достигли корня, добавляем организацию
            try {
    ...
    // Переворачиваем массив (от организации к подразделению)
    reversed_path = [];
...
if (search_param != '') {
    search_lower = StrLowerCase(search_param);
    sub_query = "for $elem in subdivisions where $elem/is_disbanded != true() return $elem";
    all_sub_list = ArraySelectAll(tools.xquery(sub_query));
    
    // ШАГ 1: Собираем ВСЕ подходящие подразделения
    potential_subs = [];
    
    for (sub_elem in all_sub_list) {
        try {
            if (!sub_elem.id || sub_elem.id == '') continue;
            ...
            // Очищаем от точек
            clean_sub_name = subName;
            while (StrContains(clean_sub_name, '.', false)) {
                clean_sub_name = StrReplace(clean_sub_name, '.', '');
            }
            clean_sub_name = Trim(clean_sub_name);
            
            // Нормализуем для сравнения
            normalized_name = normalizeSubdivisionName(clean_sub_name);
            
// Получаем оригинальное и переименованное название
original_name_lower = StrLowerCase(clean_sub_name);
renamed_name = getSubNameById(String(sub_elem.id));
renamed_name_lower = '';
if (renamed_name != null) {
    renamed_name_lower = StrLowerCase(renamed_name);
}

// Проверяем совпадение по оригинальному ИЛИ переименованному названию
name_matches = StrContains(normalized_name, search_lower, false) || 
               StrContains(original_name_lower, search_lower, false) ||
               (renamed_name_lower != '' && StrContains(renamed_name_lower, search_lower, false));

if (name_matches) {
    // НОВАЯ ПРОВЕРКА: Пропускаем скрытые префиксы
    if (startsWithHiddenPrefix(clean_sub_name)) {
        result.GetOptProperty('debug').push('Skip hidden prefix in search: ' + clean_sub_name);
        continue;
    }
    
    has_collab_recursive = hasCollaboratorsRecursive(String(sub_elem.id), 0);
    has_children_with_collab = hasSubdivisionsWithCollaborators(String(sub_elem.id));
    
    if (!has_collab_recursive && !has_children_with_collab) continue;
    
    // Используем ПЕРЕИМЕНОВАННОЕ название если есть
    display_name_for_search = renamed_name != null ? renamed_name : clean_sub_name;
    
    // Добавляем в потенциальные
    sub_info = new Object();
    sub_info.SetProperty('id', String(sub_elem.id));
    sub_info.SetProperty('name', String(sub_elem.name));
    sub_info.SetProperty('clean_name', display_name_for_search); // ВАЖНО: используем переименованное
    sub_info.SetProperty('normalized_name', normalizeSubdivisionName(display_name_for_search));
    sub_info.SetProperty('org_id', String(sub_elem.org_id));
    sub_info.SetProperty('parent_object_id', String(sub_elem.parent_object_id));
    
    potential_subs.push(sub_info);
}
        } catch(subErr) {}
    }
    
    // ШАГ 2: СНАЧАЛА собираем ВСЕ группы и запоминаем ID
    grouped_map = new Object();
    grouped_ids_set = new Object(); // ID которые в группах
    
    for (pot_sub in potential_subs) {
        try {
            pot_sub_id = pot_sub.GetOptProperty('id', '');
            pot_parent_id = pot_sub.GetOptProperty('parent_object_id', '');
            
            if (pot_parent_id == '' || pot_parent_id == null) {
                continue; // Пока пропускаем, обработаем потом
            }
            
            // Проверяем родителя
            parent_query = "for $elem in subdivisions where $elem/id = " + pot_parent_id + " return $elem";
            parent_result = ArraySelectAll(tools.xquery(parent_query));
            
            
            
            parent_name = getSubNameById(pot_parent_id);
            if (parent_name == null) {
                parent_name = String(parent_result[0].name);
            }
            
            // Если родитель скрытый - группируем
            if (startsWithHiddenPrefix(parent_name)) {
                norm_key = pot_sub.GetOptProperty('normalized_name', '');
                
                if (!grouped_map.HasProperty(norm_key)) {
                    grouped_map.SetProperty(norm_key, []);
                }
                
                group_item = new Object();
                group_item.SetProperty('id', pot_sub_id);
                group_item.SetProperty('name', pot_sub.GetOptProperty('name', ''));
                group_item.SetProperty('clean_name', pot_sub.GetOptProperty('clean_name', ''));
                group_item.SetProperty('org_id', pot_sub.GetOptProperty('org_id', ''));
                group_item.SetProperty('parent_object_id', pot_parent_id);
                group_item.SetProperty('regional_parent_name', parent_name);
                
                grouped_map.GetOptProperty(norm_key).push(group_item);
                
                // ЗАПОМИНАЕМ ID
                grouped_ids_set.SetProperty(pot_sub_id, true);
            }
        } catch(sepErr) {}
    }
    
    // ШАГ 3: Формируем grouped элементы
    for (norm_key in grouped_map) {
        try {
            group_items = grouped_map.GetOptProperty(norm_key);
            
            if (ArrayCount(group_items) == 0) continue;
            // Удалив это - поиск пока стал стабильнее чем был
            // Если только 1 элемент - НЕ группируем, убираем из set
            /*if (ArrayCount(group_items) == 1) {
                single_item = group_items[0];
                grouped_ids_set.DeleteProperty(single_item.GetOptProperty('id', ''));
                continue;
            }*/
            
            // Несколько элементов - создаем grouped
            first_item = group_items[0];
            display_name = first_item.GetOptProperty('clean_name', '');
            
            grouped_ids_str = '';
            for (g_idx = 0; g_idx < ArrayCount(group_items); g_idx++) {
                if (g_idx > 0) grouped_ids_str = grouped_ids_str + ',';
                grouped_ids_str = grouped_ids_str + group_items[g_idx].GetOptProperty('id', '');
            }
            
            // Проверяем дочерние
            has_real_children = false;
            for (g_idx = 0; g_idx < ArrayCount(group_items); g_idx++) {
                if (hasSubdivisionsWithCollaborators(group_items[g_idx].GetOptProperty('id', ''))) {
                    has_real_children = true;
                    break;
                }
            }
            ...
            grouped_data = new Object();
            grouped_data.SetProperty('id', 'grouped_' + norm_key);
            grouped_data.SetProperty('name', display_name);
            grouped_data.SetProperty('type', 'grouped');
            grouped_data.SetProperty('is_grouped', true);
            grouped_data.SetProperty('group_count', ArrayCount(group_items));
            grouped_data.SetProperty('has_subdivisions', has_real_children);
            grouped_data.SetProperty('grouped_items_str', grouped_ids_str);
            grouped_data.SetProperty('children', []);
            grouped_data.SetProperty('collaborators', []);
    ...
    // ШАГ 4: Теперь добавляем НЕгруппированные подразделения
    filtered_sub_list = [];
    all_grouped_ids = new Object();

for (norm_key in grouped_map) {
    try {
        group_items = grouped_map.GetOptProperty(norm_key);
        for (g_idx = 0; g_idx < ArrayCount(group_items); g_idx++) {
            group_id = group_items[g_idx].GetOptProperty('id', '');
            parent_id = group_items[g_idx].GetOptProperty('parent_object_id', '');
            
            // Добавляем сам ID
            all_grouped_ids.SetProperty(group_id, true);
            
            // Добавляем родителя (скрытое подразделение)
            if (parent_id != '' && parent_id != null) {
                all_grouped_ids.SetProperty(parent_id, true);
            }
        }
    } catch(e) {}
}
for (pot_sub in potential_subs) {
    try {
        ...
        if (pot_parent_id != '' && pot_parent_id != null && all_grouped_ids.HasProperty(pot_parent_id)) {
            result.GetOptProperty('debug').push('SKIP: ' + pot_sub_id + ' - parent in group');
            continue;
        }
        
if (pot_parent_id != '' && pot_parent_id != null && all_grouped_ids.HasProperty(pot_parent_id)) {
    // Проверяем есть ли дочерние подразделения
    has_children_check_query = "for $elem in subdivisions where $elem/parent_object_id = " + pot_sub_id + " and $elem/is_disbanded != true() return $elem";
    has_children_check_result = ArraySelectAll(tools.xquery(has_children_check_query));
    
    if (ArrayCount(has_children_check_result) == 0) {
        // Нет дочерних - пропускаем
        result.GetOptProperty('debug').push('SKIP: ' + pot_sub_id + ' - parent in group and no children');
        continue;
    } else {
        // Есть дочерние - НЕ пропускаем (это может быть дивизион)
        result.GetOptProperty('debug').push('KEEP: ' + pot_sub_id + ' - parent in group BUT has children');
    }
}
        
        sub_full_query = "for $elem in subdivisions where $elem/id = " + pot_sub_id + " return $elem";
        sub_full_result = ArraySelectAll(tools.xquery(sub_full_query));
        
        if (ArrayCount(sub_full_result) > 0) {
            filtered_sub_list.push(sub_full_result[0]);
        }
    } catch(normErr) {
        result.GetOptProperty('debug').push('ERROR filtering: ' + String(normErr));
    }
}
    // Обрабатываем обычные подразделения
    for (sub_elem in filtered_sub_list) {
        try {
            sub_data = new Object();
            sub_data.SetProperty('id', String(sub_elem.id));
            subName = getSubNameById(String(sub_elem.id));
    ...
    structure_array = ArraySort(structure_array, 'name', '+');
}
        ...
        temp_org_array = []; // Временный массив
    ...
    try {
        ...
        temp_org_array = []; // Временный массив для сортировки
        ...
        for (org_elem in org_list) {
            try {
                ...
                org_data.SetProperty('children', []);
                org_data.SetProperty('collaborators', []);
                temp_org_array.push(org_data); // Добавляем во временный массив
            } catch(orgItemErr) {}
        }
        ...
        // Добавляем отсортированные организации в структуру
        for (org in sorted_org_array) {
